...
# so a primeira. Usado para `java -version`, cuja 2a/3a linha normalmente
# identifica a VM/vendor, relevante para proveniencia experimental.
...
# fixar nem alterar nenhum parametro de GC usado pelo benchmark.
function Get-JavaGC() {
...
# PowerShell nao trata automaticamente codigo de saida != 0 de executavel
# nativo como erro terminante, mesmo com $ErrorActionPreference = "Stop".
...
# Valida todas as toolchains extras solicitadas antes de criar o staging e
# antes de executar o nucleo. Pedido explicito impossivel falha cedo.
...
# trabalho temporario sob o mesmo out/, promovido para $FinalDir somente
# apos toda a execucao e scripts\validate_run.py terem sucesso.
...
# vazio faria Move-Item colocar o staging dentro dele.
...
# Promocao: so alcancada se tudo acima teve sucesso.
